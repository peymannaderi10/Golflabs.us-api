import { ScheduleConfig, GeneratedSession } from './league.types';
 ...
 */
export function generateSessionDates(config: ScheduleConfig): GeneratedSession[] {
  validateConfig(config);

  let sessions: GeneratedSession[];
  switch (config.pattern) {
    case 'weekly':
      sessions = generateWeekly(config, 7); break;
    case 'biweekly':
      sessions = generateWeekly(config, 14); break;
    case 'multi_day_weekly':
      sessions = generateMultiDayWeekly(config); break;
    case 'daily_block':
      sessions = generateDailyBlock(config); break;
    case 'custom':
      sessions = generateCustom(config); break;
    default:
...
export const previewSchedule = generateSessionDates;
...
function validateConfig(config: ScheduleConfig): void {
  ...
  for (const d of config.daysOfWeek) {
    if (d < 0 || d > 6) throw new Error(`Invalid day of week: ${d}`);
  }
 ...
 */
function generateWeekly(config: ScheduleConfig, stepDays: number): GeneratedSession[] {
  const totalWeeks = config.totalCalendarWeeks ?? 10;
 ...
 */
function generateMultiDayWeekly(config: ScheduleConfig): GeneratedSession[] {
  const totalWeeks = config.totalCalendarWeeks ?? 10;
  if (totalWeeks <= 0) return [];
 ...
 */
function generateDailyBlock(config: ScheduleConfig): GeneratedSession[] {
 ...
 */
function generateCustom(config: ScheduleConfig): GeneratedSession[] {
