...
export const SYSTEM_DEFAULTS: Record<DocumentType, string> = {
...
export class DocumentService {
  async getActiveDocuments(locationId: string): Promise<ActiveDocumentSet | null> {
    ...
    if (error) {
      logger.error({ err: error, locationId }, 'Error fetching active documents');
  ...
  async getDocumentHistory(locationId: string, documentType: DocumentType): Promise<LocationDocument[]> {
    ...
    if (error) {
      logger.error({ err: error, locationId, documentType }, 'Error fetching document history');
      throw new Error('Failed to fetch document history');
    }

    return (data || []) as LocationDocument[];
  }
  ...
  async publishDocument(params: {
    ...
    if (error) {
      logger.error({ err: error, locationId, documentType }, 'Error publishing document');
      throw new Error('Failed to publish document');
    }

    logger.info(
      { locationId, documentType, version: data.version, publishedBy },
      'Document published'
    );
