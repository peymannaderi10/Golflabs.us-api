...
export class AgreementService {
  async recordAgreements(params: RecordAgreementsParams) {
    const {
      userId,
      signerName,
      signerEmail,
    ...
    const missingAgreements = REQUIRED_AGREEMENT_TYPES.filter(
      (type) => !agreements.includes(type)
    );
  ...
  async checkAgreements(userId: string, bookingId: string) {
    if (!userId || !bookingId) {
    ...
    const allAccepted = REQUIRED_AGREEMENT_TYPES.every((type) =>
    ...
    return {
      allAccepted,
      accepted: data || [],
      missing: REQUIRED_AGREEMENT_TYPES.filter(
   ...
   */
  static hashDocument(text: string): string {
