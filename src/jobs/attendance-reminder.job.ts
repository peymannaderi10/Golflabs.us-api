 ...
 */
export async function sendAttendanceReminders(): Promise<void> {
  try {
    ...
    const now = new Date();
    const frontendUrl = resendConfig.frontendUrl;
    ...
    for (const league of leagues) {
      try {
        ...
        const rows = await attendanceService.generateAttendanceRows(league.id, week.id);
