 ...
 */
export async function processAttendanceCutoffs(): Promise<void> {
  try {
      ...
      try {
        ...
        for (const week of weeks) {
          try {
            ...
            if (!attendanceRows || attendanceRows.length === 0) continue;
            if (attendanceRows[0].locked) continue; // Already locked
            ...
            if (now < cutoffTime) continue; // Not time yet
            ...
            if (league.format === 'team' && league.team_min_attendance) {
              const allAttendance = await attendanceService.getAttendanceForWeek(week.id);
              // Group by team
              const teamMap = new Map<string, { confirmed: number; total: number }>();
              for (const row of allAttendance) {
                const teamId = row.league_team_id;
                if (!teamId) continue;
                if (!teamMap.has(teamId)) teamMap.set(teamId, { confirmed: 0, total: 0 });
                const team = teamMap.get(teamId)!;
                team.total++;
                if (row.status === 'confirmed') team.confirmed++;
              }
